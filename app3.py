...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col">
    ...
    <main class="flex-1 p-6 max-w-7xl mx-auto w-full space-y-6">
        ...
        <form method="POST" class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center bg-slate-900/60 p-4 rounded-2xl border border-slate-800 gap-4">
                <div class="text-xs text-slate-400 max-w-xl text-center sm:text-left leading-relaxed">
                    ⚙️ <strong>Arsitektur Sinkron:</strong> Anda bisa memodifikasi kode di kolom kiri secara langsung, lalu menekan tombol patch untuk menyimulasikan bagaimana mesin <em>Fuzzy Sliding Window</em> bekerja merombak perubahan parsial.
                </div>
        ...
        {% if log_status %}
        <section class="p-4 rounded-xl border {% if log_status.startswith('SUKSES') or log_status.startswith('BERHASIL') %} bg-emerald-950/30 border-emerald-500/40 text-emerald-400 {% else %} bg-rose-950/30 border-rose-500/40 text-rose-400 {% endif %} text-sm mono flex items-center space-x-2 shadow-md">
...
def fuzzy_sliding_window_patch(isi_asli, search_block, replace_block, threshold=0.70):
    ...
    if best_ratio >= threshold:
        ...
        log_msg = f"SUKSES menemukan target dengan tingkat akurasi kecocokan fuzzy {round(best_ratio * 100, 2)}% pada baris ke-{best_match_index + 1}."
...
def index():
    ...
    if request.method == 'POST':
        ...
        # Ambil modifikasi kode manual dari user secara realtime
        ...
        if action == 'save_manual':
            ...
            pattern = r"<<<<<<<\s*SEARCH[\r\n]+([\s\S]*?)[\r\n]+=======\s*[\r\n]+([\s\S]*?)[\r\n]+>>>>>>>\s*REPLACE"
            match = re.search(pattern, ai_instruction_cleaned)
            
            if match:
                search_block = match.group(1)
                replace_block = match.group(2)
                
                isi_terupdate, log_status = fuzzy_sliding_window_patch(
                    FILE_ASLI_SIMULASI, search_block, replace_block
                )
                
                if isi_terupdate is not None:
                    diff = unified_diff(
                        [l.rstrip('\r') for l in FILE_ASLI_SIMULASI.splitlines()],
                        [l.rstrip('\r') for l in isi_terupdate.splitlines()],
                        lineterm=""
                    )
                    diff_results = list(diff)[2:]
                    FILE_ASLI_SIMULASI = isi_terupdate
            else:
